...
extension ElementView {
	struct Word: Hashable, Equatable {
		...
		//		let letters: ArraySlice<[SubElement]>
		
		//		func contains(subElement: Array<SubElement>.Index) -> Bool {
		//			letters.indices.contains(subElement)
		//		}
	}
...
//										}
										print("String(str)Rect=", str, bound, "\nFont: \(font.pointSize)")
										words.append(Word(string: String(str), elementID: element.id, boundingRect: bound))
									}
								}
							}
						}else {
							if let font = UIFont.init(fitting: element.string, into: rect.size, with: [:], options: []) {
									...
									print("String(str)Rect=", element.string, rect, "\nFont: \(font.pointSize)")
								if let index = words.firstIndex(where: { $0.elementID == element.id }) {
									words.remove(at: index)
								}
									words.append(Word(string: element.string, elementID: element.id, boundingRect: rect))
...
//	@GestureState var dragLocation: CGPoint = .zero
	...
	@Binding var selectedRange: [VisionRecognizedTextResult.ID: (CGFloat, CGFloat)]
		...
		Rectangle()
			...
			.overlayPreferenceValue(AnchorPreferenceKey<String>.self) { value in
				GeometryReader { proxy in
					overlaySubElementViews(preferenceValue: value, geo: proxy)
				}
			}
...
//			.border(Color.red)
	...
	func overlaySubElementViews(preferenceValue value: AnchorPreferenceKey<String>.Value, geo: GeometryProxy) -> some View {
...
//			print("SubCount: \(subElements.count)\nStringLength: \(element.string.count)")
		}
...
//					let increment = elementBounds.width / CGFloat(trimmed.count)
					if let font = UIFont.init(fitting: word.string, into: word.boundingRect.size, with: [:], options: []) {
						WordView(word: $model.words[idx], dragLocation: $dragLocation, font: font, size: word.boundingRect.size, highlightRangeInX: { (min, max) in
							selectedRange.removeAll()
							selectedRange[element.id] = (min, max)
						}, deselectRangeInX: {
							selectedRange[element.id] = nil
						})
//							.background(Color(word.highlighted ? .black : .clear))
					}
				}
			}
		}
